...
class SearchBill extends Component {
    ...
    renderError() {
        return (
            <div className='clear-left margin-left'>
                <h3 className='font-condensed-bold'>THERE ARE NO TRANSACTIONS FOR THIS DATE</h3>
            </div>
        )
    }
    ...
    onSubmit(values) {
        ...
        console.log(moment(values.release_date).format("YYYY-MM-DD HH:mm:ss"));


        API.getBills(moment(values.release_date).format("YYYY-MM-DD"))
            .then((res) => {
